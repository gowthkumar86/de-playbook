...
export const Section02View: React.FC<Section02ViewProps> = ({
  activePartId,
  onSelectPart,
  completedParts,
  ...
  const currentPart =
    SECTION_02_PARTS.find((p) => p.id === activePartId) || SECTION_02_PARTS[0];
  const isPartCompleted = completedParts.includes(currentPart.id);

  const currentPartIndex = SECTION_02_PARTS.findIndex((p) => p.id === currentPart.id);
  const prevPart = currentPartIndex > 0 ? SECTION_02_PARTS[currentPartIndex - 1] : null;
  const nextPart =
    currentPartIndex < SECTION_02_PARTS.length - 1
      ? SECTION_02_PARTS[currentPartIndex + 1]
      : null;
  ...
  const getPartQuestions = (partId: string): InterviewQuestion[] => {
    switch (partId) {
      ...
      case 'sql-part-02':
        return WINDOW_FUNCTIONS_INTERVIEW_QUESTIONS;
      case 'sql-part-03':
        return PATTERNS_INTERVIEW_QUESTIONS;
      case 'sql-part-04':
        return OPTIMIZATION_INTERVIEW_QUESTIONS;
      case 'sql-part-05':
        return CHEATSHEET_INTERVIEW_QUESTIONS;
      default:
        return ALL_SECTION_02_QUESTIONS;
    }
  };
      ...
      <div className="max-w-5xl mx-auto py-8 px-4 md:px-8">
        ...
        <div className="mb-6 p-2 rounded-xl bg-[#FAF7F2] dark:bg-[#1E1B17] border border-[#D9D1C1] dark:border-[#38332B] flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-xs text-[#5A5245] dark:text-[#A89F91]">
              49 Advanced SQL Tiered Questions
            </span>
          </div>
          {onSelectStudyMode && (
        ...
        <div className="mb-6 text-center">
          <h1 className="text-2xl md:text-3xl font-serif-heading font-bold text-[#1A1A1A] dark:text-[#EDE8DF]">
            Advanced SQL 49 Tiered Q&A Flashcard Deck
          </h1>
          <p className="text-xs text-[#5A5245] dark:text-[#A89F91] mt-1">
            Rehearse your answers aloud. Reveal the Senior standard to refine mechanical nuance and edge cases.
          </p>
        </div>
        <FlashcardMode questions={ALL_SECTION_02_QUESTIONS} />
      </div>
    );
  }
      ...
      <div className="max-w-5xl mx-auto py-8 px-4 md:px-8">
        ...
        <div className="mb-6 p-2 rounded-xl bg-[#FAF7F2] dark:bg-[#1E1B17] border border-[#D9D1C1] dark:border-[#38332B] flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-xs text-[#5A5245] dark:text-[#A89F91]">
              Advanced SQL Senior Readiness Evaluation
            </span>
          </div>
          {onSelectStudyMode && (
            <div className="flex items-center space-x-1.5 self-start sm:self-auto">
        ...
        <div className="mb-6 text-center">
          <h1 className="text-2xl md:text-3xl font-serif-heading font-bold text-[#1A1A1A] dark:text-[#EDE8DF]">
            Advanced SQL Senior Readiness Exam
          </h1>
          <p className="text-xs text-[#5A5245] dark:text-[#A89F91] mt-1">
            Rate yourself honestly across all 49 questions to uncover gaps before your senior technical loops.
          </p>
        </div>
        <QuizMode
          questions={ALL_SECTION_02_QUESTIONS}
          scores={quizScores}
        ...
        <div className="flex items-center space-x-2.5">
          ...
          <div>
            ...
            <span className="text-xs font-semibold text-[#1A1A1A] dark:text-[#EDE8DF]">
              Select learning format for Section 02
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mt-6 pt-4 border-t border-[#E9E4D9] dark:border-[#38332B] text-xs text-[#5A5245] dark:text-[#A89F91]">
          <div className="flex items-center space-x-4">
            ...
            <span className="flex items-center space-x-1">
              ...
              <span>Runnable SQL Examples</span>
            </span>
          </div>
                ...
                <div className="mt-8 pt-6 border-t border-[#E9E4D9] dark:border-[#2D2821] space-y-6">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-mono-code font-bold uppercase text-[#BF360C]">
                      {currentQuestions.length} Tiered Interactive Question Cards
                    </span>
                    <span className="text-[11px] text-[#8C7B65]">
                      Click tabs to compare Basic / Strong / Senior tiers
                    </span>
                  </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {prevPart ? (
            ...
            >
              ...
              <div>
                <span className="text-[10px] font-mono-code uppercase text-[#8C7B65] block">
                  Previous Part ({prevPart.partNumber})
                </span>
            ...
            <a
              ...
              onClick={(e) => {
                e.preventDefault();
                window.history.pushState(null, '', '/section/01');
                window.dispatchEvent(new PopStateEvent('popstate'));
                window.scrollTo({ top: 0, behavior: 'smooth' });
              }}
              className="p-4 rounded-xl border border-[#D9D1C1] dark:border-[#38332B] bg-[#FFFFFF] dark:bg-[#1A1815] hover:bg-[#FAF7F2] dark:hover:bg-[#25201A] transition-colors text-left flex items-start space-x-3 cursor-pointer group"
            >
              ...
              <div>
                ...
                <span className="text-sm font-semibold text-[#1A1A1A] dark:text-[#EDE8DF] group-hover:text-[#BF360C] transition-colors">
                  ← Section 01: Snowflake Architecture & Engineering
                </span>
              </div>
            </a>
          )}
